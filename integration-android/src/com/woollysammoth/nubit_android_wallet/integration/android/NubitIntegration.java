...
package com.woollysammoth.nubit_android_wallet.integration.android;
 ...
 */
public final class NubitIntegration
{
	...
	private static final String MIMETYPE_PAYMENTREQUEST = "application/nubit-paymentrequest"; // BIP 71

	/**
	 * Request any amount of Nubit (probably a donation) from user, without feedback from the app.
	 * 
	 * @param context
	 *            Android context
	 * @param address
	...
	{
		final Intent intent = makeNubitUriIntent(address, null);

		start(context, intent);
	}

	/**
	 * Request specific amount of Nubit from user, without feedback from the app.
	 * 
	 * @param context
	 *            Android context
	 * @param address
	...
	/**
	 * Request any amount of Nubit (probably a donation) from user, with feedback from the app. Result intent can be
	 ...
	 * 
	 * Warning: A success indication is no guarantee! To be on the safe side, you must drive your own Nubit
	...
	/**
	 * Put BIP70 payment message into result intent. Meant for usage by Nubit wallet applications.
	 * 
	 * @param result
	 *            result intent
	 * @param payment
	...
	/**
	 * Get BIP70 payment message from result intent. Meant for usage by applications initiating a Nubit payment.
	 * 
	 * You can use the transactions contained in the payment to validate the payment. For this, you need your own
	 * Nubit infrastructure though. There is no guarantee that the payment will ever confirm.
	 * 
	 * @param result
	 *            result intent
	...
	/**
	 * Put transaction hash into result intent. Meant for usage by Nubit wallet applications.
	 * 
	 * @param result
	 *            result intent
	 * @param txHash
	...
	/**
	 * Get transaction hash from result intent. Meant for usage by applications initiating a Nubit payment.
	 * 
	 * You can use this hash to request the transaction from the Nubit network, in order to validate. For this, you
	 * need your own Nubit infrastructure though. There is no guarantee that the transaction has ever been broadcasted
	...
	private static Intent makeNubitUriIntent(final String address, final Long amount)
	{
	...
	{
		Toast.makeText(context, "No Nubit application found.\nPlease install Nubit Wallet.", Toast.LENGTH_LONG).show();

		final Intent marketIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("market://details?id=com.woollysammoth.nubit_android_wallet"));
		final Intent binaryIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("https://github.com/MatthewLM/nubit-android-wallet"));
