...
import com.woollysammoth.nubitj.core.DumpedPrivateKey;
...
import com.woollysammoth.nubitj.protocols.payments.PaymentRequestException.PkiVerificationException;
...
import com.woollysammoth.nubit_android_wallet.util.Io;
...
public abstract class InputParser
{
	...
	public abstract static class StringInputParser extends InputParser
	{
		...
		public void parse()
		{
			...
			else if (input.startsWith("ppcoin:"))
			{
				try
				{
					final NubitURI nubitUri = new NubitURI(null, input);

					final Address address = nubitUri.getAddress();
					if (address == null)
						throw new NubitURIParseException("missing address");

					if (address.getParameters().equals(Constants.NETWORK_PARAMETERS))
						handlePaymentIntent(PaymentIntent.fromNubitUri(nubitUri));
					else
						error(R.string.input_parser_invalid_address, input);
				}
				catch (final NubitURIParseException x)
				{
					log.info("got invalid nubit uri: '" + input + "'", x);

					error(R.string.input_parser_invalid_nubit_uri, input);
				}
			}
			else if (PATTERN_Nubit_ADDRESS.matcher(input).matches())
			{
				try
				{
	...
	private static final Pattern PATTERN_Nubit_ADDRESS = Pattern.compile("[" + new String(Base58.ALPHABET) + "]{20,40}");
