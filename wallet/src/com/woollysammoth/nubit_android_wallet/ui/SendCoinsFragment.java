...
import org.nubit.protocols.payments.Protos.Payment;
...
import com.woollysammoth.nubit_android_wallet.AddressBookProvider;
import com.woollysammoth.nubit_android_wallet.Configuration;
import com.woollysammoth.nubit_android_wallet.Constants;
import com.woollysammoth.nubit_android_wallet.ExchangeRatesProvider;
import com.woollysammoth.nubit_android_wallet.ExchangeRatesProvider.ExchangeRate;
...
import com.woollysammoth.nubit_android_wallet.data.PaymentIntent.Standard;
import com.woollysammoth.nubit_android_wallet.integration.android.NubitIntegration;
import com.woollysammoth.nubit_android_wallet.offline.DirectPaymentTask;
import com.woollysammoth.nubit_android_wallet.ui.InputParser.BinaryInputParser;
import com.woollysammoth.nubit_android_wallet.ui.InputParser.StreamInputParser;
import com.woollysammoth.nubit_android_wallet.ui.InputParser.StringInputParser;
import com.woollysammoth.nubit_android_wallet.util.Bluetooth;
import com.woollysammoth.nubit_android_wallet.util.GenericUtils;
import com.woollysammoth.nubit_android_wallet.util.Nfc;
import com.woollysammoth.nubit_android_wallet.util.PaymentProtocol;
import com.woollysammoth.nubit_android_wallet.util.WalletUtils;
...
public final class SendCoinsFragment extends SherlockFragment
{
				...
				if (missing != null)
					msg.append(
							getString(R.string.send_coins_fragment_insufficient_money_msg1,
									NBTPrefix + ' ' + GenericUtils.formatValue(missing, NBTPrecision, NBTShift))).append("\n\n");
				if (pending.signum() > 0)
					msg.append(
							getString(R.string.send_coins_fragment_pending,
									NBTPrefix + ' ' + GenericUtils.formatValue(pending, NBTPrecision, NBTShift))).append("\n\n");
	...
	private void handleEmpty()
	{
		...
		amountCalculatorLink.setNBTAmount(available);
	...
	private void updateView()
	{
		if (paymentIntent != null)
		{
			...
			if (sentTransaction != null)
			{
				...
				sentTransactionListAdapter.setPrecision(NBTPrecision, NBTShift);
	...
	private void initStateFromNubitUri(@Nonnull final Uri nubitUri)
	{
		final String input = nubitUri.toString();
	...
	private void initStateFromIntentUri(@Nonnull final String mimeType, @Nonnull final Uri nubitUri)
	{
		try
		{
			final InputStream is = contentResolver.openInputStream(nubitUri);
	...
	private void updateStateFrom(final @Nonnull PaymentIntent paymentIntent)
	{
		...
		handler.post(new Runnable()
		{
			...
			public void run()
			{
				if (state == State.INPUT)
				{
					amountCalculatorLink.setNBTAmount(paymentIntent.getAmount());
